...
- `ZK_SERVER_HEAP` — the server maximum heap in MiB, or `auto` to omit `-Xmx`.
- `SERVER_JVMFLAGS` — additional JVM options for the server.
- `ZK_CLIENT_HEAP` — the client maximum heap in MiB, or `auto` to omit `-Xmx`.
...
| `auto`           | No generated `-Xmx` | No generated `-Xmx` |

Use `auto` when JVM memory sizing should be configured with other options. For
