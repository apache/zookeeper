...
- _watch_details/wchd_ :
  ...
  "watch_kind" is an array ordered as "data", then "children". Entries with
  the same path, session, and mode are merged, so Standard Data and Children
  Watches on the same znode are returned as `["data", "children"]`. Persistent
  and Persistent Recursive Watches also cover both kinds.
