 ...
 *
 * This implementation uses the prometheus-java-client library (version 1.x)
 * and exposes metrics via an embedded Jetty server.
 *
 * @since 3.6.0
 */
public class PrometheusMetricsProvider implements MetricsProvider {
    ...
    public void configure(Properties configuration) throws MetricsProviderLifeCycleException {
        ...
        if (httpPort == -1 && httpsPort == -1) {
            throw new MetricsProviderLifeCycleException("Either httpPort or httpsPort must be configured for Prometheus exporter.");
        }
    ...
    public void start() throws MetricsProviderLifeCycleException {
        ...
        try {
            ...
            LOG.info("Prometheus metrics provider with Jetty started. HTTP port: {}, HTTPS port: {}",
                httpPort != -1 ? httpPort : "disabled",
                httpsPort != -1 ? httpsPort : "disabled");
    ...
    private SslContextFactory.Server createSslContextFactory() {
        ...
        if (this.needClientAuth && (this.trustStorePath == null || this.trustStorePath.isEmpty())) {
             throw new IllegalArgumentException("'" + SSL_NEED_CLIENT_AUTH + "' is true, but '" + SSL_TRUSTSTORE_LOCATION + "' is not set.");
        }
     ...
     * @param acceptors The number of acceptor threads.
     * @param selectors The number of selector threads.
     * @param sslContextFactory The pre-configured SslContextFactory.
     * @return A configured ServerConnector for HTTPS.
     */
    private ServerConnector createSslConnector(Server server, int acceptors, int selectors, SslContextFactory.Server sslContextFactory) {
    ...
    /**
     * Returns a Prometheus servlet for integration with existing web applications.
    ...
    /**
     * Inner class implementing the MetricsContext interface.
     * It handles the creation and registration of different metric types.
     */
    private class Context implements MetricsContext {

        private final ConcurrentMap<String, PrometheusCounterWrapper> counters = new ConcurrentHashMap<>();
        private final ConcurrentMap<String, PrometheusLabelledCounterWrapper> counterSets = new ConcurrentHashMap<>();
        private final ConcurrentMap<String, GaugeWithCallback> registeredGauges = new ConcurrentHashMap<>();
        private final ConcurrentMap<String, PrometheusSummaryWrapper> basicSummaries = new ConcurrentHashMap<>();
        private final ConcurrentMap<String, PrometheusSummaryWrapper> advancedSummaries = new ConcurrentHashMap<>();
        private final ConcurrentMap<String, PrometheusLabelledSummaryWrapper> basicSummarySets = new ConcurrentHashMap<>();
        private final ConcurrentMap<String, PrometheusLabelledSummaryWrapper> advancedSummarySets = new ConcurrentHashMap<>();
        ...
        public Counter getCounter(String name) {
            return counters.computeIfAbsent(name, key -> {
                io.prometheus.metrics.core.metrics.Counter prometheusCounter = io.prometheus.metrics.core.metrics.Counter.builder().name(key).help(key + " counter").register(registry);
        ...
        public CounterSet getCounterSet(String name) {
            return counterSets.computeIfAbsent(name, key -> {
                ...
                io.prometheus.metrics.core.metrics.Counter prometheusCounter = io.prometheus.metrics.core.metrics.Counter.builder()
                    .name(key)
                    .help(key + " counter set")
                    .labelNames(LABEL)
                    .register(registry);
        ...
        public void registerGaugeSet(final String name, final GaugeSet gaugeSet) {
            ...
            GaugeWithCallback newGauge = GaugeWithCallback.builder()
                .name(name)
                .help(name)
                .labelNames(LABEL)
                .callback(callback -> {
        ...
        private io.prometheus.metrics.core.metrics.Summary createPrometheusSummary(String name, DetailLevel detailLevel, String... labelNames) {
            io.prometheus.metrics.core.metrics.Summary.Builder builder = io.prometheus.metrics.core.metrics.Summary.builder()
                .name(name)
                .help(name + " summary")
                .quantile(0.5, 0.05); // Median

            if (detailLevel == DetailLevel.ADVANCED) {
                builder.quantile(0.95, 0.01)  // 95th percentile
        ...
        public Summary getSummary(String name, DetailLevel detailLevel) {
            ConcurrentMap<String, PrometheusSummaryWrapper> map = detailLevel == DetailLevel.BASIC ? basicSummaries : advancedSummaries;
            return map.computeIfAbsent(name, key -> {
                if ((detailLevel == DetailLevel.BASIC && advancedSummaries.containsKey(key))
                    ...
                    throw new IllegalArgumentException("Already registered a summary as " + key + " with a different detail level");
                }
        ...
        public SummarySet getSummarySet(String name, DetailLevel detailLevel) {
            ConcurrentMap<String, PrometheusLabelledSummaryWrapper> map = detailLevel == DetailLevel.BASIC ? basicSummarySets : advancedSummarySets;
            return map.computeIfAbsent(name, key -> {
                if ((detailLevel == DetailLevel.BASIC && advancedSummarySets.containsKey(key))
                    ...
                    throw new IllegalArgumentException("Already registered a summary set as " + key + " with a different detail level");
                }
                io.prometheus.metrics.core.metrics.Summary prometheusSummary = createPrometheusSummary(key, detailLevel, LABEL);
    ...
    private static class PrometheusLabelledCounterWrapper implements CounterSet {
        ...
        public void add(String key, long delta) {
            try {
                ...
                LOG.error("invalid delta {} for metric {} with key {}", delta, prometheusCounter.getPrometheusName(), key, e);
            }
        }
