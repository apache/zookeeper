...
/**
 * Handles the logic of converting a PrometheusRegistry scrape result
            ...
            if (isAdvanced) {
                ...
                for (Quantile quantile : quantiles) {
                    ...
                    switch (quantileValue) {
                        case "0.5":
                            result.put(buildKeyForDump(metricName, dataPoint.getLabels().add("quantile", quantileValue)), quantile.getValue());
                            break;
                        case "0.95":
                            result.put(buildKeyForDump(metricName, dataPoint.getLabels().add("quantile", quantileValue)), quantile.getValue());
                            break;
