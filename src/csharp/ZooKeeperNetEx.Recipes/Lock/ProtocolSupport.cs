...
namespace org.apache.zookeeper.recipes.@lock
{
	...
	public class ProtocolSupport
	{
		...
		protected readonly ZooKeeper zookeeper;
		private readonly ThreadSafeInt closed = new ThreadSafeInt(0);
		private const int retryDelayInMs = 500;
	    private const int retryCount = 10;
	    protected List<ACL> m_acl = ZooDefs.Ids.OPEN_ACL_UNSAFE;

	    protected ProtocolSupport(ZooKeeper zookeeper)
		{
		...
		/// return type. </returns>
		protected bool retryOperation(ZooKeeperOperation operation)
		{
		...
		{
			ensureExists(path, null, m_acl, CreateMode.PERSISTENT);
		}
		...
		private void ensureExists(string path, byte[] data, List<ACL> acl, CreateMode flags)
		{
			try
			{
				retryOperation(new ZooKeeperOperationAnonymousInnerClassHelper(this, path, data, acl, flags));
			}
		...
		private sealed class ZooKeeperOperationAnonymousInnerClassHelper : ZooKeeperOperation
		{
			...
			public bool execute()
			{
				Stat stat = outerInstance.zookeeper.exists(path, false);
				if (stat != null)
				{
					return true;
				}
				outerInstance.zookeeper.create(path, data, acl, flags);
				return true;
			}
		}
		...
		private static void retryDelay(int attemptCount)
		{
			if (attemptCount > 0)
			{
				try
				{
                    Thread.Sleep(attemptCount * retryDelayInMs);
				}
				catch (ThreadInterruptedException e)
				{
