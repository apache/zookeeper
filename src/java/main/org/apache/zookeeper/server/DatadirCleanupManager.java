...
public class DatadirCleanupManager {
  ...
  void diskCleanUp(String mountPoint) {

    List<String> livePools = helixAdmin.getResourcesInCluster(clusterName);
    Accessor accessor = new Accessor(helixManager);
    List<String> storePools = accessor.getAllPools();

    for (String poolName : storePools) {

      if (!livePools.contains(poolName)) {

        String diskCleanUpPath = mountPoint;
        try {
          FileUtils.deleteDirectory(new File(diskCleanUpPath));
        } catch (IOException e) {
          LOG.error("Failed to delete directory " + diskCleanUpPath, e);
        }
      }
    }
