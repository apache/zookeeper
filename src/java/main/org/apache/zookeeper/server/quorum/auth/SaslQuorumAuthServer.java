...
public class SaslQuorumAuthServer implements QuorumAuthServer {
    ...
    public SaslQuorumAuthServer(boolean quorumRequireSasl, String loginContext)
        ...
        try {
            ...
            SaslServerCallbackHandler saslServerCallbackHandler = new SaslServerCallbackHandler(
                    Configuration.getConfiguration(), loginContext);
        ...
        try {
            if (!QuorumAuth.nextPacketIsAuth(din)) {
                if (quorumRequireSasl) {
                    throw new SaslException("Client not trying to authenticate"
            ...
            if (ss.isComplete()) {
                ...
                LOG.info("Successfully completed the authentication using SASL. client addr: {}",
            ...
            if (quorumRequireSasl) {
                ...
                LOG.warn("Maintaining client connection despite SASL authentication failure."
                        + " server addr: {} quorum.auth.serverRequireSasl: {}",
                        sock.getRemoteSocketAddress(), quorumRequireSasl);
