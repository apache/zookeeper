...
public class QuorumSocketFactory {
    ...
    public ServerSocket buildForServer(final QuorumPeerConfig quorumPeerConfig,
                                       final int listenPort,
                                       final InetAddress bindAddr)
            throws X509Exception, IOException {
        return buildForServer(quorumPeerConfig, listenPort, LISTEN_BACKLOG, bindAddr);
    }

    public ServerSocket buildForServer(final QuorumPeerConfig quorumPeerConfig,
                                       final int port)
            throws X509Exception, IOException {
        return buildForServer(quorumPeerConfig, port, LISTEN_BACKLOG, null);
    }
    ...
    public ServerSocket buildForServer(final QuorumPeerConfig quorumPeerConfig,
        ...
        ServerSocket s = null;
        if (this.sslEnabled) {
            s = newSslServerSocket(quorumPeerConfig, listenPort, backlog, bindAddr);
        } else {
    ...
    public Socket buildForClient(final QuorumPeerConfig quorumPeerConfig)
            throws X509Exception, IOException {
        if (this.sslEnabled) {
            return newSslSocket(quorumPeerConfig);
        } else {
            return newSocket();
        }
    }
    ...
    private Socket newSslSocket(final QuorumPeerConfig quorumPeerConfig)
            throws X509Exception, IOException {
        Socket clientSocket = null;
        try {
            clientSocket = QuorumX509Util.createSSLContext(quorumPeerConfig)
                    .getSocketFactory()
                    .createSocket();
    ...
    private ServerSocket newSslServerSocket(
            final QuorumPeerConfig quorumPeerConfig, final int port,
        ...
        try {
            if (listenAddr != null) {
                serverSocket =
                        (SSLServerSocket)QuorumX509Util.createSSLContext(quorumPeerConfig)
