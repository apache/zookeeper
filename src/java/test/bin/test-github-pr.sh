...
runCoreTests () {
  ...
  echo "$ANT_HOME/bin/ant -DZookeeperPatchProcess= -Dtest.junit.output.format=xml -Dtest.output=yes -Dtest.junit.threads=1 -Dcompile.c++=yes -Dforrest.home=$FORREST_HOME -Djava5.home=$JAVA5_HOME -Dtestcase=QuorumSSLTest -Djava.security.debug=certpath -Djavax.net.debug=ssl,handshake"
  $ANT_HOME/bin/ant -DZookeeperPatchProcess= -Dtest.junit.output.format=xml -Dtest.output=yes -Dtest.junit.threads=1 -Dcompile.c++=yes -Dforrest.home=$FORREST_HOME -Djava5.home=$JAVA5_HOME -Dtestcase=QuorumSSLTest -Djava.security.debug=certpath -Djavax.net.debug=ssl,handshake
  if [[ $? != 0 ]] ; then
    JIRA_COMMENT="$JIRA_COMMENT
