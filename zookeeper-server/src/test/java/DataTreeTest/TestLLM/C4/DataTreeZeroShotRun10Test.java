...
public class DataTreeZeroShotRun10Test {
    ...
    @Test
    public void testCopyStatPersistedDeprecated() {
        StatPersisted from = new StatPersisted();
        from.setCzxid(12345L);
        StatPersisted to = new StatPersisted();
        DataTree.copyStatPersisted(from, to);
        assertEquals(12345L, to.getCzxid());
    }

    @Test
    public void testCopyStatDeprecated() {
        Stat from = new Stat();
        from.setCzxid(54321L);
        Stat to = new Stat();
        DataTree.copyStat(from, to);
        assertEquals(54321L, to.getCzxid());
    }

    @Test
    public void testCreateStatStaticMethod() {
        StatPersisted stat = DataTree.createStat(10L, 20L, 30L);
        assertEquals(10L, stat.getCzxid());
        assertEquals(20L, stat.getCtime());
        assertEquals(30L, stat.getEphemeralOwner());
    }
}