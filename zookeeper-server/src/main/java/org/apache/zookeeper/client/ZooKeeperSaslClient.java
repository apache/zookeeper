...
/**
 * This class manages SASL authentication for the client. It allows ClientCnxn to authenticate using
 * SASL with a ZooKeeper server.
 */
public class ZooKeeperSaslClient {
  /**
   * * Use {@link ZKClientConfig#LOGIN_CONTEXT_NAME_KEY} instead.
  */
  @Deprecated
  public static final String LOGIN_CONTEXT_NAME_KEY = "zookeeper.sasl.clientconfig";
  /**
   * Use {@link ZKClientConfig#ENABLE_CLIENT_SASL_KEY} instead.
   * */
  @Deprecated
  public static final String ENABLE_CLIENT_SASL_KEY = "zookeeper.sasl.client";
  /**
   * Use {@link ZKClientConfig#ENABLE_CLIENT_SASL_DEFAULT} instead.
   * */
  @Deprecated
  ...
  /**
   * Returns true if the SASL client is enabled. By default, the client is enabled but can be
   * disabled by setting the system property <code>zookeeper.sasl.client</code> to <code>false
   * </code>. See ZOOKEEPER-1657 for more information.
   *
  ...
  public static boolean isEnabled() {
    return Boolean.valueOf(
        System.getProperty(
            ZKClientConfig.ENABLE_CLIENT_SASL_KEY, ZKClientConfig.ENABLE_CLIENT_SASL_DEFAULT));
  }
  ...
  /**
   * the ZooKeeperSaslClient Constructor.
   */
  public ZooKeeperSaslClient(final String serverPrincipal, ZKClientConfig clientConfig)
      throws LoginException {
    /**
     * ZOOKEEPER-1373: allow system property to specify the JAAS configuration section that the
     * zookeeper client should use. Default to "Client".
     */
    ...
    try {
      ...
      // third party customized getAppConfigurationEntry could throw IllegalArgumentException when
      // JAAS
      // configuration isn't set. We can reevaluate whether to catch RuntimeException instead when
      // more
    ...
    if (entries != null) {
      this.configStatus =
          "Will attempt to SASL-authenticate using Login Context section '" + clientSection + "'";
      this.saslClient = createSaslClient(serverPrincipal, clientSection);
    } else {
      // Handle situation of clientSection's being null: it might simply because the client does not
      // intend to
      // use SASL, so not necessarily an error.
      saslState = SaslState.FAILED;
      String explicitClientSection =
          clientConfig.getProperty(ZKClientConfig.LOGIN_CONTEXT_NAME_KEY);
      if (explicitClientSection != null) {
        // If the user explicitly overrides the default Login Context, they probably expected SASL
        // to
        ...
        if (runtimeException != null) {
          ...
          throw new LoginException(
              "Client cannot SASL-authenticate because the specified JAAS configuration "
                  + "section '"
                  + explicitClientSection
                  + "' could not be found.");
        }
      } else {
        // The user did not override the default context. It might be that they just don't intend to
        // use SASL,
  ...
  public static class ServerSaslResponseCallback implements AsyncCallback.DataCallback {
    ...
    /**
     the callback method of the ServerSaslResponseCallback.
     */
  ...
  private SaslClient createSaslClient(final String servicePrincipal, final String loginContext)
    ...
    try {
      if (!initializedLogin) {
        synchronized (this) {
          if (login == null) {
            ...
            // connections.
            // in order to ensure the login is initialized only once, it must be synchronized the
            // code snippet.
            login =
                new Login(
                    loginContext, new SaslClientCallbackHandler(null, "Client"), clientConfig);
      ...
      return SecurityUtils.createSaslClient(
          login.getSubject(), servicePrincipal, "zookeeper", "zk-sasl-md5", LOG, "Client");
  ...
  /**
   * respond to Server with the serverToken.
   */
  public void respondToServer(byte[] serverToken, ClientCnxn cnxn) {
    ...
    if (saslClient.isComplete()) {
      ...
      // enable the socket's writable flag so that any packets waiting for authentication to
      // complete in
  ...
  private byte[] createSaslToken(final byte[] saslToken) throws SaslException {
    if (saslToken == null) {
      ...
      throw new SaslException(
          "Error in authenticating with a Zookeeper Quorum member: "
    ...
    if (subject != null) {
      synchronized (login) {
        try {
          ...
          if (e.toString().contains(UNKNOWN_SERVER_ERROR_TEXT)) {
            error +=
                " This may be caused by Java's being unable to resolve the "
                    + "Zookeeper Quorum Member's"
                    + " hostname correctly. You may want to try to adding"
                    + " '-Dsun.net.spi.nameservice.provider.1=dns,sun' to "
                    + "your client's JVMFLAGS environment.";
          }
      ...
      throw new SaslException(
          "Cannot make SASL token without subject defined. "
              + "For diagnosis, please look for WARNs and ERRORs "
              + "in your log related to the Login class.");
    }
  }
  ...
  /**
   * used by ClientCnxn to know whether to emit a SASL-related event: either AuthFailed or
   * SaslAuthenticated,
  ...
  */
  ...
  /**
   Initialize the client's communications with the Zookeeper server by sending the server the
   first authentication packet.
   */
  ...
  /**
   * return whether the client Authentication is in the Progress.
   * @return true in the Progress,otw:false.
   */
