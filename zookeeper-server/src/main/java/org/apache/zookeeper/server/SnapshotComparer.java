...
public class SnapshotComparer {
    ...
    private void usage() {
        ...
        help.printHelp(120, "java -cp <classPath> " + SnapshotComparer.class.getName(), "", options, "");
    }
    ...
    private static class TreeInfo {
        ...
        TreeInfo(File snapshot) throws Exception {
            ...
            System.out.println(String.format(
                    "Processed data tree in %f seconds", ((((double) end - beginning) / 1000000)) / 1000));
        }
    ...
    private static DataTree getSnapshot(File file) throws Exception {
        ...
        System.out.println(String.format(
                ...
                file.getName(), (((double) (end - beginning) / 1000000)) / 1000));
        return dataTree;
    }
    ...
    private static void compareTrees(
            TreeInfo left, TreeInfo right, int byteThreshold, int nodeThreshold, boolean debug, boolean interactive) {
                ...
                if (input.isEmpty()) {
                    ...
                    if (input.startsWith("/")) {
                        ...
                        try {
                            ...
                            // input is invalid
                            System.out.println(String.format(
                                    "Input %s is not valid. Depth must be in range [%d, %d]. Path must be an absolute path which starts with '/'.",
                                    input, 0, maxDepth - 1));
                        }
                    }
                }
    ...
    private static void compareLine(
        ...
        List<TreeInfo.TreeNode> leftList =
                depth >= left.nodesAtDepths.size() ? new ArrayList<>() : left.nodesAtDepths.get(depth);
        List<TreeInfo.TreeNode> rightList =
                depth >= right.nodesAtDepths.size() ? new ArrayList<>() : right.nodesAtDepths.get(depth);
    ...
    static void printLeftOnly(
            TreeInfo.TreeNode node, int byteThreshold, int nodeThreshold, boolean debug, boolean interactive) {
    ...
    static void printBoth(
        ...
        if (Math.abs(rightNode.descendantSize - leftNode.descendantSize) > byteThreshold
            ...
            System.out.println(String.format(
                    ...
                    leftNode.label, leftNode.descendantSize, rightNode.descendantSize));
        }
    }

    static void printNode(TreeInfo.TreeNode node, StringBuilder builder) {
        builder.append(
                String.format("Descendant size: %d. Descendant count: %d", node.descendantSize, node.descendantCount));
    }
}
