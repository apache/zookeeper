...
public class DataTree {
    ...
    public DataNode getNode(String path) {
        LOG.info("Got node " + path);
        DataNode node = nodes.get(path);
        dataCache.put(path, node, getNodeSize(path, node.data));
        return node;
    }
...
//            trackNewlyAddedNodeForCacheControl(path, getNodeSize(path, data));
    ...
    public Stat setData(String path, byte data[], int version, long zxid,
        ...
        DataNode node = nodes.get(path);
        dataCache.update(path, node, getNodeSize(path, node.data));
...
//        prepareNodesForAccessFromDataCache(path);
     ...
     */
    void serializeNode(OutputArchive oa, StringBuilder path, ArrayList<String> cacheList) throws IOException {
        String pathString = path.toString();
        DataNode node = getNode(pathString);
        if (node == null) {
            return;
        }
        node.setCachePosition(cacheList.indexOf(pathString));
        String children[] = null;
        DataNode nodeCopy;
        synchronized (node) {
        ...
        for (String child : children) {
            ...
            path.append(child);
            serializeNode(oa, path, cacheList);
        }
    }
    ...
    public void serialize(OutputArchive oa, String tag) throws IOException {
        ...
        ArrayList<String> cacheList = dataCache.toArrayList();
        serializeNode(oa, new StringBuilder(""), cacheList);
        // / marks end of stream
    ...
    public void deserialize(InputArchive ia, String tag) throws IOException {
        LOG.info("DESERIALIZING DATA TREE");
        aclCache.deserialize(ia);
        nodes.clear();
        pTrie.clear();
        nodeDataSize.set(0);
        String path = ia.readString("path");
        ArrayList<DataCacheRecord> cacheList = new ArrayList<>();
        // ***************************************************
        // CSCI 612 - Blue
        //
        // We need to only deserialize a certain number of nodes here and add them to the
        // data cache.
        //
        // Tyler
        // ***************************************************
        while (!"/".equals(path)) {
            ...
            // CSCI 612 - Blue
            //
            // Adds the deserialized node to the data cache.
            //
            // Tyler
            // ***************************************************
            if (node.cachePosition >= 0) {
                LOG.info("Deserialized CACHED node " + path);
                cacheList.add(new DataCacheRecord(path, node, getNodeSize(path, node.data)));
            }
            synchronized (node) {
            ...
            if (lastSlash == -1) {
                ...
                LOG.info("Attempted to get parent node " + parentPath);
                if (parent == null) {
                    throw new IOException("Invalid Datatree, unable to find " +
                            "parent " + parentPath + " of path " + path);
                }
                parent.addChild(path.substring(lastSlash + 1));
                LOG.info("Added child " + path.substring(lastSlash + 1) + " to parent " + parentPath);
                long eowner = node.stat.getEphemeralOwner();
                EphemeralType ephemeralType = EphemeralType.get(eowner);
                if (ephemeralType == EphemeralType.CONTAINER) {
                    containers.add(path);
        ...
        cacheList.sort(Comparator.comparingInt(r -> r.node.cachePosition));
        cacheList.forEach(r -> dataCache.put(r.path, r.node, getNodeSize(r.path, r.node.data)));
...
//        boolean nodeInCacheAlready = dataCache.put(path);
