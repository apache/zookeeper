    ...
    private void setOrCheckOpKind(Op.OpKind ok) throws IllegalArgumentException {
        if (opKind == null) {
            ...
            throw new IllegalArgumentException(
                    "Mixing read and write operations (transactions)" + " is not allowed in a multi request.");
        }
    }
            ...
            while (!h.getDone()) {
                switch (h.getType()) {
                    ...
                    case ZooDefs.OpCode.createContainer:
                        ...
                        CreateOptions options = CreateOptions.newBuilder(cr.getAcl(), createMode)
                                .build();
                    ...
                    case ZooDefs.OpCode.createTTL:
                        ...
                        add(Op.create(
                                crTtl.getPath(), crTtl.getData(), crTtl.getAcl(), crTtl.getFlags(), crTtl.getTtl()));
                        break;
                    case ZooDefs.OpCode.delete:
