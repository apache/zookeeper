...
// Adapted from org.apache.zookeeper.server.admin.UnifiedConnectionFactory (Jetty 9)
// for Jetty 12 API changes (SslConnection ctor, SocketAddress, getSslEndPoint).
...
public class UnifiedConnectionFactory extends AbstractConnectionFactory {
    ...
    public Connection newConnection(Connector connector, EndPoint realEndPoint) {
        ...
        if (isSSL) {
            SocketAddress remoteAddr = aheadEndpoint.getRemoteSocketAddress();
            SSLEngine engine = (remoteAddr instanceof InetSocketAddress)
                ? this.sslContextFactory.newSSLEngine((InetSocketAddress) remoteAddr)
                : this.sslContextFactory.newSSLEngine();
            engine.setUseClientMode(false);
    ...
    protected SslConnection newSslConnection(
        ...
        return new SslConnection(connector.getByteBufferPool(), connector.getExecutor(),
            sslContextFactory, endPoint, engine);
    }
