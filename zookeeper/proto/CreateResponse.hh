...
#ifndef CreateResponse_HH_
#define CreateResponse_HH_
...
class CreateResponse: public ERecord {
public:
	EString path;

	CreateResponse() {
	}
	CreateResponse(EString path) {
		this->path = path;
	}
	EString getPath() {
		return path;
	...
	virtual int compareTo(EObject* peer_) THROWS(EClassCastException) {
		CreateResponse* peer = dynamic_cast<CreateResponse*>(peer_);
		if (!peer) {
			return false;
		}
		if (peer_ == this) {
