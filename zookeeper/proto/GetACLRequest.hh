...
#ifndef GetACLRequest_HH_
#define GetACLRequest_HH_
...
class GetACLRequest: public ERecord {
public:
	EString path;

	GetACLRequest() {
	}
	GetACLRequest(EString path) {
		this->path = path;
	}
	EString getPath() {
		return path;
	...
	virtual int compareTo(EObject* peer_) THROWS(EClassCastException) {
		GetACLRequest* peer = dynamic_cast<GetACLRequest*>(peer_);
		if (!peer) {
			return false;
		}
		if (peer_ == this) {
