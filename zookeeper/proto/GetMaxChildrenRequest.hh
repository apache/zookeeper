...
#ifndef GetMaxChildrenRequest_HH_
#define GetMaxChildrenRequest_HH_
...
class GetMaxChildrenRequest: public ERecord {
public:
	EString path;

	GetMaxChildrenRequest() {
	}
	GetMaxChildrenRequest(EString path) {
		this->path = path;
	}
	EString getPath() {
		return path;
	...
	virtual int compareTo(EObject* peer_) THROWS(EClassCastException) {
		GetMaxChildrenRequest* peer = dynamic_cast<GetMaxChildrenRequest*>(peer_);
		if (!peer) {
			return false;
		}
		if (peer_ == this) {
...
#endif /* GetMaxChildrenRequest_HH_ */
