...
#ifndef SetACLResponse_HH_
#define SetACLResponse_HH_
...
class SetACLResponse: public ERecord {
public:
	sp<Stat> stat;

	SetACLResponse() {
	}
	SetACLResponse(sp<Stat> stat) {
		this->stat = stat;
	}
	sp<Stat> getStat() {
		return stat;
	...
	virtual int compareTo(EObject* peer_) THROWS(EClassCastException) {
		SetACLResponse* peer = dynamic_cast<SetACLResponse*>(peer_);
		if (!peer) {
