...
#ifndef SyncResponse_HH_
#define SyncResponse_HH_
...
class SyncResponse: public ERecord {
public:
	EString path;

	SyncResponse() {
	}
	SyncResponse(EString path) {
		this->path = path;
	}
	EString getPath() {
		return path;
	...
	virtual int compareTo(EObject* peer_) THROWS(EClassCastException) {
		SyncResponse* peer = dynamic_cast<SyncResponse*>(peer_);
		if (!peer) {
			return false;
		}
		if (peer_ == this) {
