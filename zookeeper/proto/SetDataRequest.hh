...
#ifndef SetDataRequest_HH_
#define SetDataRequest_HH_
...
class SetDataRequest: public ERecord {
public:
	EString path;
	sp<EA<byte> > data;
	int version;
	...
	SetDataRequest(EString path, sp<EA<byte> > data, int version) {
		this->path = path;
		this->data = data;
	...
	virtual int compareTo(EObject* peer_) THROWS(EClassCastException) {
		SetDataRequest* peer = dynamic_cast<SetDataRequest*>(peer_);
		if (!peer) {
