...
#ifndef GetChildrenRequest_HH_
#define GetChildrenRequest_HH_
...
class GetChildrenRequest: public ERecord {
public:
	EString path;
	boolean watch;
	...
	GetChildrenRequest(EString path, boolean watch) {
		this->path = path;
		this->watch = watch;
	}
	EString getPath() {
	...
	virtual int compareTo(EObject* peer_) THROWS(EClassCastException) {
		GetChildrenRequest* peer = dynamic_cast<GetChildrenRequest*>(peer_);
		if (!peer) {
