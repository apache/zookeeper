...
#ifndef SetACLRequest_HH_
#define SetACLRequest_HH_
...
class SetACLRequest: public ERecord {
public:
	EString path;
	sp<EList<sp<ACL> > > acl;
	int version;
	...
	SetACLRequest(EString path, sp<EList<sp<ACL> > > acl, int version) {
		this->path = path;
		this->acl = acl;
	...
	virtual boolean equals(EObject* peer_) {
		SetACLRequest* peer = dynamic_cast<SetACLRequest*>(peer_);
		if (!peer) {
			return false;
		}
		if (peer_ == this) {
