...
#ifndef ErrorResponse_HH_
#define ErrorResponse_HH_
...
class ErrorResponse: public ERecord {
public:
	int err;

	ErrorResponse() {
	}
	ErrorResponse(int err) {
		this->err = err;
	}
	int getErr() {
		return err;
	...
	virtual int compareTo(EObject* peer_) THROWS(EClassCastException) {
		ErrorResponse* peer = dynamic_cast<ErrorResponse*>(peer_);
		if (!peer) {
