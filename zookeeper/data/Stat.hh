...
public:
	...
	Stat(llong czxid, llong mzxid, llong ctime, llong mtime, int version,
			int cversion, int aversion, llong ephemeralOwner, int dataLength,
			int numChildren, llong pzxid) {
		this->czxid = czxid;
		this->mzxid = mzxid;
		this->ctime = ctime;
		this->mtime = mtime;
	...
	void setDataLength(int m_) {
		dataLength = m_;
	}
	int getNumChildren() {
		return numChildren;
	}
	void setNumChildren(int m_) {
		numChildren = m_;
	}
	llong getPzxid() {
		return pzxid;
	...
	virtual int compareTo(EObject* peer_) THROWS(EClassCastException) {
		Stat* peer = dynamic_cast<Stat*>(peer_);
		if (!peer) {
		...
		ret = (cversion == peer->cversion) ?
				0 : ((cversion < peer->cversion) ? -1 : 1);
		if (ret != 0)
			return ret;
		ret = (aversion == peer->aversion) ?
				0 : ((aversion < peer->aversion) ? -1 : 1);
		if (ret != 0)
			return ret;
		ret = (ephemeralOwner == peer->ephemeralOwner) ?
				0 : ((ephemeralOwner < peer->ephemeralOwner) ? -1 : 1);
		if (ret != 0)
			return ret;
		ret = (dataLength == peer->dataLength) ?
				0 : ((dataLength < peer->dataLength) ? -1 : 1);
		if (ret != 0)
			return ret;
		ret = (numChildren == peer->numChildren) ?
				0 : ((numChildren < peer->numChildren) ? -1 : 1);
		if (ret != 0)
			return ret;
