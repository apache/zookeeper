...
#ifndef SetMaxChildrenTxn_HH_
#define SetMaxChildrenTxn_HH_
...
class SetMaxChildrenTxn: public ERecord {
public:
	EString path;
	int max;

	SetMaxChildrenTxn() : max(0) {
	}
	SetMaxChildrenTxn(EString path, int max) {
		this->path = path;
		this->max = max;
	}
	EString getPath() {
	...
	virtual int compareTo(EObject* peer_) THROWS(EClassCastException) {
		SetMaxChildrenTxn* peer = dynamic_cast<SetMaxChildrenTxn*>(peer_);
		if (!peer) {
