...
#ifndef CheckVersion_HH_
#define CheckVersion_HH_
...
class CheckVersionTxn: public ERecord {
public:
	EString path;
	int version;

	CheckVersionTxn() : version(0) {
	}
	CheckVersionTxn(EString path, int version) {
		this->path = path;
	...
	virtual int compareTo(EObject* peer_) THROWS(EClassCastException) {
		CheckVersionTxn* peer = dynamic_cast<CheckVersionTxn*>(peer_);
		if (!peer) {
