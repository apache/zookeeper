 ...
 *
 * This callback is used to decrypt the content of password file into the actual
 * password.
 *
 ...
 *   character of the actual password, otherwise the behavior is undefined.
 ...
 *
 * This structure holds the parameters for getting the actual SASL password.
 */
 ...
 * \brief allocates and initializes a basic array of Cyrus SASL callbacks.
 *
 * This small helper function is similar to zoo_sasl_make_basic_callbacks, 
 * except that the actual password would be generated by a user-defined
 * processor.
 *
 * \param user the "canned" response to \c SASL_CB_USER and \c SASL_CB_AUTHNAME,
 *   or NULL for none
 * \param realm the "canned" response to \c SASL_CB_GETREALM, or NULL for none
 * \param password the name of a file whose first line is read in
 *   response to \c SASL_CB_PASS, or NULL for none
 * \return the freshly-malloc()ed callbacks array, or NULL if allocation
 *   failed. Deallocate with free(), but only after the corresponding
 *   ZooKeeper handle is closed.
